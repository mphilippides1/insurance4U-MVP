  ...
  return (
    <section className="bg-yellow-50 py-24">
      <div className="max-w-6xl mx-auto px-6">
        <h3 className="text-5xl font-extrabold text-center mb-20 text-yellow-600">Why Insurance4U?</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          {features.map((feature, index) => (
            <motion.div
              key={index}
              className="bg-white p-10 rounded-3xl shadow-xl border border-yellow-100 flex flex-col items-center text-center hover:shadow-2xl transition"
              animate={{ y: [0, -10, 0] }}
              transition={{ duration: 4, delay: index * 0.3, repeat: Infinity, repeatType: "reverse" }}
              whileHover={{ scale: 1.05 }}
            >
              <div className="text-6xl mb-6">{feature.icon}</div>
              <h4 className="text-2xl font-bold mb-3 text-yellow-700">{feature.title}</h4>
